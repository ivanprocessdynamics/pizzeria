          ...
          {images.map((src, index) => (
            <AnimateIn key={index} direction="bottom" delay={0.1 * index}>
              <motion.div style={{ y: yTransforms[index % yTransforms.length] }} className="overflow-hidden rounded-lg shadow-lg relative h-full w-full">
                <ImageWithSkeleton src={src} alt={`Galería de Pizzería ${index + 1}`} eager />
              </motion.div>
            </AnimateIn>
          ))}
        </div>
...
const ImageWithSkeleton: React.FC<ImageWithSkeletonProps> = ({ src, alt, eager }) => {
    ...
    const run = async () => {
      ...
      if (isIstockPage(src)) {
        try {
          ...
          if (resolved) list = [resolved, buildProxy(resolved), ...baseCandidates];
        } catch {}
      }
      let idx = 0;
      const tryNext = () => {
